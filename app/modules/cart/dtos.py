...
class CartItemWithProductDTO(BaseModel):
    ...
    Combines data from:
    - tbl_cart_items: user_id, product_id, quantity, price_cart, total, dates
    - tbl_products: title, sku, price_product
    """

    # Cart item fields (from tbl_cart_items)
    ...
    price_cart: float = Field(..., ge=0, description="Price per item when added to cart (from tbl_cart_items)")
    ...
    # Product fields (from tbl_products via JOIN)
    title: str = Field(..., description="Product title (current)")
    sku: str = Field(..., description="Product SKU (current)")
    price_product: float = Field(..., ge=0, description="Current product price (from tbl_products)")
